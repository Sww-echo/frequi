  ...
  <div class="flex h-full">
    <div class="flex-fill w-full flex-col align-items-stretch flex h-full">
      <div class="flex me-0 items-center md:gap-2 gap-1 flex-wrap mx-1 md:mx-0">
        ...
        <div class="flex gap-1 md:gap-2 w-full md:w-auto">
          <BaseStringMultiSelectMenu
            ...
            :placeholder="$t('chart.selectPairs')"
            virtualize
            size="md"
          />
          <USelectMenu
        ...
        <div class="ms-auto flex flex-wrap items-center gap-2">
          <BaseCheckbox v-model="settingsStore.showMarkArea">
            <span class="text-nowrap">{{ $t('chart.showAreas') }}</span>
          </BaseCheckbox>
      ...
      <div v-else class="flex flex-col items-center justify-center h-full w-full">
        <span class="text-2xl font-semibold">{{ $t('chart.noPair') }}</span>
      </div>
    </div>
    <DraggableModal
      v-model:open="showPlotConfigModal"
      :title="$t('chart.configTitle')"
      class="max-w-xl"
      :description="$t('chart.configDescription')"
      :overlay="false"
      :modal="false"
      :dismissible="false"
    >
