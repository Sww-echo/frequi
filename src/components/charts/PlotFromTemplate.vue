...
<template>
  <div v-if="visible" class="pt-1">
    <UFormField v-if="!showIndicatorMapping" :label="$t('chart.selectTemplate')" class="text-md">
      <UListbox
        id="selectTemplate"
    ...
    <div v-else>
      <h5 class="mt-1 text-center text-md mb-1">{{ $t('chart.remapIndicators') }}</h5>
      <div
    ...
    <div class="mt-2 flex gap-1 justify-end">
      <UButton :title="$t('chart.abort')" color="neutral" icon="mdi:close" @click="visible = false" />
      <UButton
        v-if="!showIndicatorMapping"
        :disabled="!selTemplateName"
        :title="$t('chart.useTemplate')"
        :label="` ${$t('chart.useTemplate')}`"
        class="w-40"
        variant="solid"
        icon="mdi:check"
      ...
      <UButton
        v-if="showIndicatorMapping"
        :disabled="!selTemplateName"
        :title="$t('chart.applyTemplate')"
        class="w-40"
        variant="solid"
        icon="mdi:check"
        @click="fromTemplateApply"
        :label="$t('chart.applyTemplate')"
      />
    </div>
  </div>
</template>
