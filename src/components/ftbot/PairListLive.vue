...
<template>
  <div>
    <div>
      <h3 class="text-xl">{{ $t('pairlist.whitelistMethods') }}</h3>
    ...
    <!-- Show Whitelist -->
    <h3 class="text-lg font-bold" :title="t('pairlist.pairCount', { count: botStore.activeBot.whitelist.length })">
      {{ $t('pairlist.whitelist') }}
    </h3>
    ...
    <p v-else>{{ $t('pairlist.unavailable') }}</p>
    <USeparator />
    ...
    <div>
      <div class="flex flex-row justify-center mb-1">
        <label
          ...
          :title="$t('pairlist.selectBlacklist')"
        >
          ...
          <UPopover>
            ...
            <template #content>
              <form ref="form" @submit.prevent="addBlacklistPair" class="py-2 px-3">
                <div class="space-y-1">
                  <h4 class="font-bold mb-2">{{ $t('pairlist.addBlacklist') }}</h4>
                  <UFormField :label="$t('trade.detail.pair')" class="space-x-2" required>
                    <UInput v-model="newblacklistpair" required autofocus></UInput>
                  </UFormField>
                  <UButton id="blacklist-submit" class="ms-auto mb-2" type="submit">{{ $t('pairlist.add') }}</UButton>
                </div>
              </form>
            </template>
          </UPopover>
    ...
    <p v-else>
      {{ $t('pairlist.noBlacklist') }}<br />
      {{ $t('pairlist.noBlacklistHint') }}
    </p>
  </div>
</template>
