...
<template>
  <div>
    <UAlert
      color="info"
      class="mb-3 py-2"
      :title="$t('analysis.lookahead')"
      :description="$t('analysis.lookaheadLongDescription')"
    />

    <div class="flex flex-col gap-3">
      <div>
        <span class="font-bold">{{ $t('analysis.strategy') }}</span>
      ...
      <div class="grid grid-cols-1 md:grid-cols-2 gap-3 items-center">
        <div class="flex items-center gap-2 md:justify-end">
          <label for="lookahead-minimum-trade-amount" class="font-bold"
            >{{ $t('analysis.minimumTradeAmount') }}</label
          >
          <InfoBox
            :hint="$t('analysis.minimumTradeHint')"
          />
        </div>
        <UInput
      ...
      <div class="grid grid-cols-1 md:grid-cols-2 gap-3 items-center">
        <div class="flex items-center gap-2 md:justify-end">
          <label for="lookahead-targeted-trade-amount" class="font-bold"
            >{{ $t('analysis.targetedTradeAmount') }}</label
          >
          <InfoBox
            :hint="$t('analysis.targetedTradeHint')"
          />
        </div>
        <UInput
      ...
      <div class="flex items-center gap-2">
        ...
        <label for="lookahead-allow-limit-orders">{{ $t('analysis.allowLimitOrders') }}</label>
        <InfoBox
          :hint="$t('analysis.limitOrdersHint')"
        />
      </div>
