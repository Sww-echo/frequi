...
async function startAnalysis(payload: LookaheadAnalysisPayload) {
  ...
  try {
    ...
    if (status.status === 'failed') {
      statusMessage.value = status.error || t('analysis.lookaheadFailed');
    ...
    if (analysis.status === 'ended') {
      ...
      statusMessage.value = analysis.status_msg || t('analysis.lookaheadFailed');
      showAlert(statusMessage.value, 'error');
    }
  } catch (error) {
    console.error(error);
    showAlert(t('analysis.lookaheadFailed'), 'error');
  } finally {
    running.value = false;
  }
}
...
<template>
  <div class="px-1 mx-auto w-full max-w-4xl lg:max-w-7xl">
    ...
    <DraggableContainer :header="$t('analysis.lookahead')" class="mx-1 p-4">
      <LookaheadAnalysisForm :running="running" @start="startAnalysis" />
    </DraggableContainer>
    <DraggableContainer v-if="result" :header="$t('analysis.result')" class="mx-1 mt-4 p-4">
