...
const combinedPairList = computed(() => {
  ...
  props.pairlist.forEach((pair) => {
    ...
    if (trades.length > 0) {
      profitString = `${t('trade.currentProfit')}: ${formatPercent(profit)}`;
    }
    if (trade) {
      profitString += `\n${t('trade.openSince')}: ${timestampms(trade.open_timestamp)}`;
    }
    if (
      filterText.value === '' ||
  ...
  <div>
    ...
    >
      <li
        ...
        :title="`${formatPriceCurrency(comb.profitAbs, botStore.activeBot.stakeCurrency, botStore.activeBot.stakeCurrencyDecimals)} - ${comb.pair} - ${t('trade.profitTradeCount', { count: comb.tradeCount })}`"
