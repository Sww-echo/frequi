...
async function startAnalysis(payload: RecursiveAnalysisPayload) {
  ...
  try {
    ...
    if (status.status === 'failed') {
      statusMessage.value = status.error || t('analysis.recursiveFailed');
    ...
    if (analysis.status === 'ended') {
      ...
      statusMessage.value = analysis.status_msg || t('analysis.recursiveFailed');
      showAlert(statusMessage.value, 'error');
    }
  } catch (error) {
    console.error(error);
    showAlert(t('analysis.recursiveFailed'), 'error');
  } finally {
    running.value = false;
  }
}
...
<template>
  <div class="px-1 mx-auto w-full max-w-4xl lg:max-w-7xl">
    ...
    <DraggableContainer :header="$t('analysis.recursive')" class="mx-1 p-4">
